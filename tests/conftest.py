"""Shared test fixtures and helpers for the native (level-3) tests.

``FakeBackend`` lives here rather than in one test module so both the
scheduler tests and the engine tests can use it without importing across
test modules (which is both an import-ordering headache and a smell).
pytest imports ``conftest`` automatically, so tests just reference
``FakeBackend`` after importing it from here.
"""
