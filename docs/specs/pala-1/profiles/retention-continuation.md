...
| 0x0015 | `SEG_PRIOR_ROOT` | 32 bytes — the derived root of the prefix `[0, seq)`: the §4.3 tree over the record hashes of records `0..seq-1` in seq order, as `consistency-proof.md` §1 defines it — always from record 0, never a window — so that one value in the newest survivor covers everything ever deleted before it, however many deletions occurred, and a `pala-consistency-proof/1` between it and any later derived root verifies the prefix without the deleted records. |
...
| D4 | **What is deleted** | Whole segments, from the front, at or after the declared retention floor. Never a partial segment; never from the middle. The newest surviving continuation record then describes all of it at once (`seq` = everything before; `SEG_PRIOR_ROOT` commits to all of it). |
